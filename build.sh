#!/usr/bin/env bash
#
# Build AeroGauge: Recompiled on Linux.
#
...
#      frontend 0016, 0024, 0025, 0026, 0027; RT64 0006, 0008, 0009, 0010, 0011). Linux needs no MinGW/D3D12
...
ROM="${ROM_FILENAME:-AeroGauge (USA).z64}"
if [ ! -f "$ROM" ]; then
    die "missing ROM: $ROM
...
PATCHES=(
    ...
    "lib/N64ModernRuntime:0013-ultramodern-sp-task-synchronous-failsoft.patch"
    ...
    "lib/RecompFrontend:0027-recompfrontend-stale-focus-recovery.patch"
...
if [[ -f aspMain.us.toml ]]; then
    ...
    log "  (aspMain.us.toml not present yet - skipping RSPRecomp; audio ucode TBD)"
fi
...
log ""
log "[5/5] Building aerogauge_modern..."
cmake --build build --target aerogauge_modern -j"$NPROC"

# --- 13. Done ---------------------------------------------------------------
EXE=build/aerogauge_modern
if [ -x "$EXE" ]; then
    log ""
    log "[done] $EXE  ($(stat -c '%y' "$EXE" 2>/dev/null || stat -f '%Sm' "$EXE"))"
    log "Run from repo root:  ./build/aerogauge_modern"
else
    warn "aerogauge_modern not found at expected path: $EXE"
fi
