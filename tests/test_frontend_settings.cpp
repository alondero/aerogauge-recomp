...
int main(int argc, char** argv) {
    try {
        ...
        for (const char* key : {"api_option", "hpfb_option", "developer_mode", "texture_pack",
                               "texture_dump", "window_width", "window_height"}) {
            require(graphics.has_option(key), "missing graphics option");
            require(!graphics.is_config_option_hidden(graphics.get_config_schema().options_by_id.at(key)), "hidden graphics option");
        }
        ...
        graphics.save_config();
        flush();
        require(window_updates == 1 && aero::config::window_size().width == 2560, "window settings");
        ...
        enhancements.set_option_value("draw_distance", 0.0);
