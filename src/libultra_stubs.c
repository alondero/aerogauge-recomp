...
// NOTE(aerogauge): the Lamborghini port carried a hand-translated __osViInit_recomp here
...
typedef struct { uint16_t button; signed char stick_x; signed char stick_y; unsigned char err_no; } AeroPad;
...
static unsigned char aero_joybus_data_crc(uint8_t* rdram, gpr addr) {
    unsigned char crc = 0;
    int i, b;
    for (i = 0; i <= 32; i++) {
        for (b = 7; b >= 0; b--) {
...
// (cmd 0x02 / 0x03) and issue them through the SI bridge (func_8007F780 -> aero_joybus_answer).
...
// a dead pak. Port of the proven legacy HLE (src/recomp/recomp_support.c aero_pak_image_format,
...
uint8_t g_aero_pak_image[0x8000];

static void aero_pak_format(void) {
    uint8_t* img = g_aero_pak_image;
    static const uint8_t id_areas[4] = { 1, 3, 4, 6 };
    int a, h, page, slot, i;
    memset(img, 0, sizeof(g_aero_pak_image));
    for (a = 0; a < 4; a++) {
...
// change after startup, and aero_joybus_answer is on the per-frame input-poll hot path, so a
// getenv() per call is wasted work. Caching the path into our own buffer also avoids holding a
// pointer into getenv's storage.
static const char* aero_pak_path(void) {
    static char cached[1024];
    static int init = 0;
    if (!init) {
        const char* p = getenv("AERO_CONTROLLER_PAK_FILE");
        if (!p || !*p) p = "aero_controller_pak.mpk";
...
// Controller-pak feature is ON by default (this is the #69 deliverable). AERO_CONTROLLER_PAK=0
// is a deliberate A/B opt-out (the old W121 "no pak" behaviour) for boot-stability debugging.
static int aero_pak_enabled(void) {
    static int cached = -1;
    if (cached < 0) {
        const char* e = getenv("AERO_CONTROLLER_PAK");
        cached = (e && e[0] == '0') ? 0 : 1;
    }
    return cached;
}
...
// the game thread during boot. The file is created on the first real save (aero_pak_save).
static void aero_pak_ensure_loaded(void) {
    static int loaded = 0;
    FILE* f;
    if (loaded) return;
    loaded = 1;
    f = fopen(aero_pak_path(), "rb");
    if (f) {
        size_t n = fread(g_aero_pak_image, 1, sizeof(g_aero_pak_image), f);
        fclose(f);
        if (n == sizeof(g_aero_pak_image)) return;             /* good image on disk */
    }
    aero_pak_format();                                         /* fresh formatted pak (memory only) */
}
...
static void aero_pak_save(void) {
    const char* path = aero_pak_path();
    char tmp[1024];
    FILE* f;
    size_t n;
    ...
    f = fopen(tmp, "wb");
    if (!f) return;
    n = fwrite(g_aero_pak_image, 1, sizeof(g_aero_pak_image), f);
    if (fclose(f) != 0 || n != sizeof(g_aero_pak_image)) { remove(tmp); return; }
...
// AERO_PAK_TRACE=1: per-frame joybus log (issue #35 diagnosis) -- shows exactly what the save
...
static int aero_pak_trace(void) {
    static int cached = -1;
    if (cached < 0) { const char* e = getenv("AERO_PAK_TRACE"); cached = (e && e[0] == '1') ? 1 : 0; }
    return cached;
}
...
extern void aero_pak_set_rumble(int on);
...
static void aero_joybus_answer(uint8_t* rdram, gpr buf, const AeroPad* pads) {
    int pos = 0, channel = 0;
    int pak_ch0 = aero_pak_enabled();  /* ch0 carries a formatted pak (#69) */
    if (pak_ch0) aero_pak_ensure_loaded();
    while (pos < 0x40) {
        ...
        if (t == 0x00 || t == 0xFD) { pos++; channel++; continue; }
        {
            ...
            if (channel < 4) {
                ...
                case 0x02: { /* pak block read (real frame: rx = 0x21 = 32 data + 1 data-CRC) */
                     ...
                     * on rx==33 keeps aero_joybus_data_crc (which always folds 32 bytes) from
                    ...
                    if (has_pak && rx == 33) {
                        ...
                        if (addr >= 0x8000u) {                 /* bank/rumble-detect region */
                            ...
                            for (k = 0; k < 32; k++)
                                MEM_B(k, resp) = (signed char)g_aero_pak_image[(addr + k) & 0x7FFFu];
                        }
                        /* present pak -> REAL (non-inverted) data CRC over the 32-byte block */
                        MEM_B(32, resp) = (signed char)aero_joybus_data_crc(rdram, resp);
                ...
                case 0x03: { /* pak block write (32-byte payload after the 2 addr bytes) */
                    ...
                    unsigned char crc = (tx >= 3) ? aero_joybus_data_crc(rdram, data) : 0;
                    if (has_pak && tx >= 3) {
                        unsigned addr = ((MEM_BU(pos + 3, buf) << 8) | MEM_BU(pos + 4, buf)) & 0xFFE0u;
                        if (addr == 0xC000u) {                 /* MOTOR control block -> SDL rumble */
                            aero_pak_set_rumble(MEM_BU(0, data) != 0);
                        } else if (addr < 0x8000u) {           /* RAM: persist into the .mpk image */
                            int k;
                            for (k = 0; k < 32; k++) g_aero_pak_image[(addr + k) & 0x7FFFu] = (uint8_t)MEM_BU(k, data);
                            aero_pak_save();
                        }
...
// Permanent test knob (AERO_INPUT_PROBE, same knob as the button probe below; asserted by
...
static void aero_pak_channel_probe(uint8_t* rdram, gpr buf) {
    static int pak_probe_logged = 0;
    int pos = 0;
    if (pak_probe_logged || !getenv("AERO_INPUT_PROBE")) return;
    while (pos < 0x40) {
        unsigned t = MEM_BU(pos, buf);
        if (t == 0xFE) return;
...
// NOTE(aerogauge): the Lamborghini port defined two ROM-specific natives below this line —
// an alSynAddPlayer timing adaptation (func_8007FFF0) and the SI controller-read bridge
// (func_8007F780) that wired aero_joybus_answer + the pak image into that game's hand-rolled
// SI kernel. Both were keyed to Lamborghini function/global addresses and were dropped from
// the base stack. When AeroGauge's SI read path is identified, wire it through
// aero_joybus_answer() above (the PIF emulation itself is game-agnostic).
