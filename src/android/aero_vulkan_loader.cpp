...
PFN_vkVoidFunction vkGetInstanceProcAddr(VkInstance instance, const char* name) {
    ...
    std::call_once(once, [] {
        void* loader = nullptr;
        const char* directory = std::getenv("AERO_ANDROID_DRIVER_DIR");
        const char* library = std::getenv("AERO_ANDROID_DRIVER_NAME");
        if (directory && library) {
            loader = adrenotools_open_libvulkan(RTLD_NOW, ADRENOTOOLS_DRIVER_CUSTOM,
                std::getenv("AERO_ANDROID_CACHE_DIR"),
                std::getenv("AERO_ANDROID_NATIVE_LIB_DIR"), directory, library, nullptr, nullptr);
        } else {
        ...
        if (!get_proc) __android_log_print(ANDROID_LOG_ERROR, "AeroGauge",
