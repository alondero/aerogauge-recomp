...
// ----- Symbol table (parsed from aerogauge.syms.toml) -----
...
void find_and_load_symbol_table() {
    ...
    candidates.emplace_back(std::filesystem::current_path() / AERO_ADDR("aerogauge.syms.toml", "aerogauge.jp.syms.toml"));
#if defined(_WIN32)
    ...
    if (n > 0 && n < MAX_PATH) {
        ...
        candidates.emplace_back(exedir / AERO_ADDR("aerogauge.syms.toml", "aerogauge.jp.syms.toml"));
        candidates.emplace_back(exedir.parent_path() / AERO_ADDR("aerogauge.syms.toml", "aerogauge.jp.syms.toml"));
    }
#else
    {
        std::filesystem::path self = std::filesystem::read_symlink("/proc/self/exe");
        candidates.emplace_back(self.parent_path() / AERO_ADDR("aerogauge.syms.toml", "aerogauge.jp.syms.toml"));
        candidates.emplace_back(self.parent_path().parent_path() / AERO_ADDR("aerogauge.syms.toml", "aerogauge.jp.syms.toml"));
    }
#endif
