...
final class DriverImport {
    ...
    static void configure(Context context) throws Exception {
        ...
        try {
            ...
            if (selected.isFile()) {
                ...
                Os.setenv("AERO_ANDROID_DRIVER_DIR", directory.getAbsolutePath() + "/", true);
                Os.setenv("AERO_ANDROID_DRIVER_NAME", library, true);
            }
            gameLock = lock;
        } catch (Exception error) {
            lock.close();
    ...
    static void releaseGameLock() {
        ...
        if (lock != null) {
            try { lock.close(); }
            catch (Exception error) { android.util.Log.w("AeroGauge", "Could not release GPU driver lock", error); }
        }
    }
