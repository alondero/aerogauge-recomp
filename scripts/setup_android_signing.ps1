# Creates a persistent release identity. Run only when configuring a NEW app key.
# Password backup uses Windows DPAPI and is readable only by this Windows user.
param(
    [string]$Repository = 'alondero/aerogauge-recomp',
    [string]$KeyDirectory = "$env:LOCALAPPDATA/AeroGaugeRecomp/Signing",
    [string]$Keytool = 'keytool'
)
$ErrorActionPreference = 'Stop'
$keyPath = Join-Path $KeyDirectory 'aerogauge-release.jks'
...
$env:AERO_SIGNING_PASSWORD = $password
try {
    $secure = ConvertTo-SecureString $password -AsPlainText -Force
    [pscredential]::new('aerogauge', $secure) | Export-Clixml -LiteralPath $credentialPath
    & $Keytool -genkeypair -keystore $keyPath -storetype PKCS12 -alias aerogauge `
        -keyalg RSA -keysize 4096 -validity 10000 -dname 'CN=AeroGauge Recompiled' `
        -storepass:env AERO_SIGNING_PASSWORD -keypass:env AERO_SIGNING_PASSWORD
    ...
    Remove-Item Env:AERO_SIGNING_PASSWORD -ErrorAction SilentlyContinue
    $password = $null
    $values = $null
