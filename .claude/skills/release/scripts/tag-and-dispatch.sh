...
# Make sure we're on main. The Lambo convention (which AeroGauge inherits:
# every commit on `main` is a merge commit) is to tag at the merge commit.
...
if [[ -z "$(git log --merges --first-parent -n 1 --format='%H' HEAD | grep -F "${HEAD_SHA}" || true)" ]]; then
  ...
  echo "         Past Lambo releases tag at the merge commit (e.g. v0.4.2 = 03e533a)." >&2
  echo "         For the inaugural release, you may tag the first-commit SHA directly:" >&2
  echo "           git tag ${VERSION} <desired-sha> && git push origin ${VERSION}" >&2
  echo "         …and skip this script's auto-dispatch. Otherwise re-run after a PR lands." >&2
  exit 1
fi
